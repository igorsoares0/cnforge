...
const DEFAULT_PLANS: Plan[] = [
  {
    ...
    price: "$0",
    period: "/mo",
    features: ["10 blocks", "2 themes", "Community support"],
  },
  {
    name: "Pro",
    price: "$29",
    period: "/mo",
    features: ["All blocks", "All themes", "Priority support"],
    popular: true,
  },
];
...
export default function Signup2({
  ...
  subtitle = "You can always change plans later.",
  formAction = "#",
  formMethod = "POST",
      ...
      <div className="mx-auto w-full max-w-lg">
        ...
        <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {plans.map((plan) => (
            <label
              key={plan.name}
              className={`relative flex cursor-pointer flex-col rounded-xl border-2 p-5 transition-colors ${
            ...
            >
              <input
                ...
                name="plan"
                value={plan.name}
                defaultChecked={plan.name === selectedPlan}
                className="sr-only"
              />
              {plan.popular && (
                <Badge className="absolute -top-2.5 right-3 border-primary/20 bg-primary/10 text-[10px] text-primary">
                  Popular
                </Badge>
              )}
